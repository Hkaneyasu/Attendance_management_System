...
public class BT_service extends Service {
    ...
    private static final String DeviceName1 ="arduino_test";
    private static final String DeviceName2 = "Galaxy S7 edge";
    private static final String PCName2 = "DESKTOP-39D7KJ3";
    private static final String PCName3 = "DESKTOP-NUAV4BV";
    private static final String PCName4 = "arduino_test";
    private static final String PCName1 = "Galaxy S7 edge";
    ...
    public int minRSSI;
    public int minRSSI2;
    public boolean allowed  = false;    //通信許可
    public boolean enter = false;       //入室時true

    private BluetoothGatt mbtGatt = null;
    private Handler btHandler = new Handler();
    public String bleRSSI;

    public float phaseRSSI ;
    public int subRSSI;
    ...
    private int CYCLE = 1;     //何分
    public int HANPUKU = 5*1;      //何秒
    public int SIZEOFRSSIs = 10;

    public int[] RSSIs = new int[SIZEOFRSSIs];
    ...
    private BluetoothAdapter.LeScanCallback mScanCallBack = new BluetoothAdapter.LeScanCallback() {
        @Override
        public void onLeScan(BluetoothDevice bluetoothDevice, int i, byte[] aByte) {
            int j;

            if(bluetoothDevice.getName() == null || !bluetoothDevice.getAddress().contains("88:83:22:96:40:F9")){
                return;
            }

            if(bluetoothDevice.getAddress().equals("88:83:22:96:40:F9")){
                setRSSIs(i);
                System.out.println(String.valueOf(NowTimeIs()) + " " + averageRSSIs(i) + "\n");
                String RSSIstr = ":";
                for(j=0;j<SIZEOFRSSIs;j++) RSSIstr += RSSIs[j]+",";
                //bleRSSI = RSSIstr +"ave="+ averageRSSIs(i) + "\n";
                bleRSSI = String.valueOf(averageRSSIs(i));
                phaseRSSI = averageRSSIs(i);
            }


            if(bluetoothDevice.getAddress().equals("28:48:43:83:C3:A1")){
                subRSSI = i;
            }
        }
    };

    //RSSIs直近５個の値を保存ー＞平均を出力
    public float averageRSSIs(int finalRSSI){
        int preAve,i;
        float sum = 0;
        boolean disorder;
        int count = 0;
        sum = 0;
        for(i=0;i<SIZEOFRSSIs;i++){
            if(RSSIs[i] == 0) continue;
            sum += RSSIs[i];
            count++;
        }
        float ret;
    ...
    public void setRSSIs(int finalRSSI){
        if (finalRSSI == RSSIs[SIZEOFRSSIs-1] && RSSIs[SIZEOFRSSIs-1] == RSSIs[SIZEOFRSSIs-2]) return;
        int i;
        for(i=0;i<SIZEOFRSSIs;i++){
            if(i == SIZEOFRSSIs-1){
                RSSIs[i] = finalRSSI;
    ...
    //講義室中央にビーコンを置く設定
    public class Open_BTserver2 extends Thread{
        public void run(){
            ...
            Log.d("BLEService","scanLE開始");
            bAdapter.startLeScan(mScanCallBack);

            phaseRSSI = -100;
            ServiceState = "Serverstart: scanning devices";

            //RSSI1回目計測成功ー＞phase1開始
            while (RSSIs[0] == 0){
                try {
                    Thread.sleep(100);
                }catch (InterruptedException e){
            ...
            if(flag){   //出ていくとき
                ...
                int exitTime1 = nowTime();
                while (phaseRSSI < -70){       //30s以上離れてるか
                    try {
                        Thread.sleep(100);
                    }catch (InterruptedException e){
                        e.printStackTrace();
                    ...
                    if(nowTime() > exitTime1+30) {      //30s以上中にいたら
                        while (true) {
                            ...
                            try {
                                ...
                                check = "connectionOK";     //connectが成功したときのみ実行される
                ...
                int enterTime1 = nowTime();
                while(phaseRSSI > -70){         //離れたら最初から
                    try {
                        Thread.sleep(100);
                    }catch (InterruptedException e){
                        e.printStackTrace();
                    ...
                    if(nowTime() > enterTime1+30){      //30s以上中にいたら
                        while (true){
                            if(check.equals("disconnect") ) break;
                            if(MainActivity.escape) return;
                            try {
    ...
    public class Open_BTserver extends Thread{
        public void run(){
            ...
            MainActivity.scanedRSSI2 = -100;
            minRSSI = MainActivity.scanedRSSI;
            minRSSI2 = MainActivity.scanedRSSI2;

            ServiceState = "Serverstart";
            while (!scanRSSI(-55)) {    //RSSIが規定値以上ならtrue
                if(check.equals("disconnect")) break;
                if(MainActivity.escape) return;
                try {
                    Thread.sleep(100);
            ...
            while (!RSSIunder(minRSSI - 5)){   //RSSIが規定値以下に落ちたら
            ...
            ServiceState = "phase3 OK";

            if(MainActivity.scanedRSSI2 < -52) enter = false;     //RSSI2が落ちた＝＞退室
            else  enter = true;
            ...
            */
            ...
            //接近（device1）
            Log.d("ぶるーとぅーすのえるいーすきゃん","phase1開始\n");
            while(!(phaseRSSI > -75)){
                try {
                    Thread.sleep(100);
                }catch (InterruptedException e){
            ...
            ServiceState = "phase1 OK";
            AppInfo = "デバイス前通過しました";
            float minLeRSSI = phaseRSSI;

            //通過
            Log.d("ぶるーとぅーすのえるいーすきゃん","phase2開始");
            while (! (phaseRSSI < minLeRSSI -5)){
                try {
                    Thread.sleep(100);
                }catch (InterruptedException e){
                    e.printStackTrace();
            ...
            while (true){
                ...
                try {
                    ...
                    e.printStackTrace();
                    try {
                        Thread.sleep(100);     //connectが失敗したら1秒後にもう一度connect
                    }catch (InterruptedException w){
                        w.printStackTrace();
                    }
                }
    ...
    public class  BT_stream extends Thread{
        public void run(){
            ...
            try {
                ...
                bOStream.write("1".getBytes());  Log.d("データ通信","1回目データ送信成功");
                ...
                while (true){   //割り込みがかかるまで通信
                    ...
                    inBytes = bInStream.read (inBuff);  Log.d("データ通信","1回目データ受信成功");
                    ...
                    if(cmd.length() > 0){
                        ...
                        MainActivity.imakita = true;
                        if(flag){   //退室時処理
/*
                            if(enter){  //enter = true => エラー
                                Log.d("BluetoothService","flag(退室) but enter(true)");
                                ServiceState = "flag(退室) but enter(true)";
                                break;
                            }
 ...
 */
                            ...
                            try {
                                bOStream.write(sendMessage.getBytes()); Log.d("データ通信","2回目データ送信成功");
                            }catch (IOException e){
                                e.printStackTrace();
                            }
/*
                            try {
                                inBytes = bInStream.read (inBuff);  Log.d("データ通信","2回目データ受信成功");
                                buff = new byte[inBytes];
                                System.arraycopy(inBuff,0,buff,0,inBytes);
                                cmd = new String(buff);
                                if(cmd.length() > 0 ) ReadData = cmd;
                            }catch (IOException e){
                                e.printStackTrace();
                            }
...
/*
                            if(!enter){  //enter = true => エラー
                                Log.d("BluetoothService","flag(入室) but enter(false)");
                                ServiceState = "flag(入室) but enter(false)";
                                break;
                            }
 */
                            ...
                            String sendMessage = koma + "," + mNUMBER + "," + "0000";
                            try {
                                bOStream.write(sendMessage.getBytes()); Log.d("データ通信","2回目データ送信成功");
                            }catch (IOException e){
                                e.printStackTrace();
    ...
    public boolean SubScan(){
        ...
        if(MainActivity.scanedRSSI2 == -100){
            bAdapter.cancelDiscovery();
            return false;
        }else {
            bAdapter.cancelDiscovery();
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                if(device.getName() != null) {
                    ...
                    if(device.getName().equals(PCName1)) {
                        int rssi = intent.getShortExtra(BluetoothDevice.EXTRA_RSSI, Short.MIN_VALUE);
                        System.out.println(String.valueOf(NowTimeIs()) + " " + rssi + "\n");
                        MainActivity.scanedRSSI = rssi;
                    }

                    if(device.getName().equals(PCName3)){
                        int rssi = intent.getShortExtra(BluetoothDevice.EXTRA_RSSI, Short.MIN_VALUE);
                        MainActivity.scanedRSSI2 = rssi;
                    }
    ...
    public int NowTimeIs(){
        Calendar cal = Calendar.getInstance();       //カレンダーを取得

        int iSecond = cal.get(Calendar.SECOND);    //分を取得
        int imSec   = cal.get(Calendar.MILLISECOND);
        int iSumT_milli = iSecond*1000 + imSec;
    ...
    public int RecordAttendance2(){
       ...
       // if(!is_allowedUpdate()) { Log.d("BluetooethService","更新が許可されていません"); allowed = false; }       //更新許可が出ない＝＞プロセス終了
        ...
        if(KomaNames[cKoma].equals("未登録")){  Log.d("BluetoothService","科目名未登録で記録されません"); allowed = false;}     //科目名未登録なら記録しない
    ...
    private Notification updateNotification(){
        ...
        String info = "接続待機中";
        String apInfo = ServiceState +","+ flag + ",r1="+MainActivity.scanedRSSI + "(m;"+ minRSSI +")" +
                "r2="+MainActivity.scanedRSSI2;
        String apInfo2 = AppInfo ;

        return builder.setContentIntent(action).
                setContentTitle("出席管理システム").
                setTicker("_").
                setContentText(apInfo2).
